...
# read back in
TopPharmCompRaw <- read_csv(file = "data/2020-11-24-TopPharmCompRaw.csv")
```
...
```{r aes-sol-01, message=FALSE, warning=FALSE}
TidyTopPharmComp %>% 
...
```{r aes-sol-02, message=FALSE, warning=FALSE}
TidyTopPharmComp %>% 
  ggplot(aes(x = market_cap_year, y = market_cap_us_bil)) + 
  geom_line() + 
  lbl_graph_01
...
```{r aes-sol-03, message=FALSE, warning=FALSE}
TidyTopPharmComp %>% 
  ggplot(aes(x = market_cap_year, y = market_cap_us_bil)) + 
...
```{r aes-sol-04-gg_trend_line, message=FALSE, warning=FALSE}
gg_trend_line <- TidyTopPharmComp %>% 
  ggplot(aes(x = market_cap_year, y = market_cap_us_bil)) + 
...
```{r aes-sol-05-gg_col_graph, message=FALSE, warning=FALSE}
TidyTopPharmComp %>% 
  group_by(company_name, stock_exch) %>% 
  summarize(
    avg_market_cap = mean(market_cap_us_bil, na.rm = TRUE)) %>% 
...
```{r aes-sol-6, message=FALSE, warning=FALSE}
TidyTopPharmComp %>% 
  group_by(company_name, stock_exch) %>% 
  summarize(
    avg_market_cap = mean(market_cap_us_bil, na.rm = TRUE)) %>% 
...
```{r label-sol-07, message=FALSE, warning=FALSE}
lbl_graph_02 <- ggplot2::labs(x = "Company", 
                              y = "Average Market cap (USD in billions)", 
                              title = "Average Market Cap (2013-2019)", 
                              subtitle = "Largest Pharmaceutical Companies", 
...
```{r lbl_graph_03, message=FALSE, warning=FALSE}
...
```{r dplyr-sol-08-TidyTopPharmComCaps, message=FALSE, warning=FALSE}
TidyTopPharmComCaps <- TidyTopPharmComp %>% 
...
```{r aes-sol-09, message=FALSE, warning=FALSE}
TidyTopPharmComCaps %>% 
  ggplot() + 
...
```{r ael-sol-10, message=FALSE, warning=FALSE}
TidyTopPharmComCaps %>% 
  ggplot() + 
...
```{r aes-sol-11, message=FALSE, warning=FALSE}
library(ggrepel)
TidyTopPharmComCaps %>% 
  ggplot(aes(x = largest_market_cap_date, 
             y = largest_market_cap_us_bil)) + 
...
```{r aes-sol-12, message=FALSE, warning=FALSE}
TidyTopPharmComCaps %>% 
  ggplot(aes(x = largest_market_cap_date, 
             y = largest_market_cap_us_bil)) + 
  geom_point(aes(color = company_type), show.legend = FALSE) + 
...
```{r aes-sol-13, message=FALSE, warning=FALSE}
TidyTopPharmComCaps %>% 
  filter(stock_exch %in% c("NASDAQ", "NYSE"), 
         company_type == "Pharma") %>% 
  ggplot(aes(x = largest_market_cap_date, 
...
```{r aes-sol-14, message=FALSE, warning=FALSE}
TidyTopPharmComCaps %>% 
  filter(stock_exch %in% c("NASDAQ", "NYSE"), 
         company_type == "Pharma") %>% 
  ggplot(aes(x = largest_market_cap_date, 
