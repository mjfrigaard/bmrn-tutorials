...
- Slides: https://mjfrigaard.github.io/data-viz-as-comm/Index.html
...
One of the most important jobs of analytic work is counting things :)

Below is an example of using the `map()` function from the `purrr` package to pass the `count()` function from `dplyr` to the character variables in `TidyApple`. 
...
Before we start looking at relationships *between* variables, we should examine each variable's underlying distribution. 

We're going to cover two graphs that display variable distributions: histograms and density plots. 
...
What if we want to see how a continuous variable is distributed across a categorical variable? We did this in the [previous lesson with a boxplot](https://mjfrigaard.github.io/data-viz-intro/Index.html#77).  
...
Now we've seen the distribution of `dir_request`, and how it varies across `trans_type`. Next we'll look at how the relationship between these two variables varies over `date`.
...
```{r lab_line_02-sol, fig.height=8, fig.width=10, out.height='100%', out.width='100%', message=FALSE, warning=FALSE}
lab_line_02 <- labs(x = "Date",
                 y = "Direction Requests",
                 title = "Direction Requests Over Time (US Cities Only)",
...
```{r show-gg_us_cities, echo=FALSE, fig.height=8, fig.width=10, out.height='100%', out.width='100%', message=FALSE, warning=FALSE}
gg_us_cities
```
...
We're going to use labels to annotate and highlight US cities between March and July of 2020.
...
Build a dataset from `TidyApple` that only has US cities, and ranges from March 1, 2020 to July 1, 2020. 

```{r USCitiesMarJul-ex, eval=FALSE}
USCitiesMarJul <-  TidyApple %>% 
  filter(geo_type == ___________ &
              country == ___________, 
...
- map `min(USCitiesMarJul$date)` and `max(USCitiesMarJul$date)` inside `c()` to `xlim`   
- map `min(USCitiesMarJul$dir_request, na.rm = TRUE)` and `max(USCitiesMarJul$dir_request, na.rm = TRUE)` inside `c()` to `ylim`
...
USCitiesMarJul %>% 
    ...
    geom_line(aes(x = date, y = dir_request, 
             ...
             size = 0.30) + 
   # coordinate system
   coord_cartesian(xlim = c(min(USCitiesMarJul$date), 
                            max(USCitiesMarJul$date)), 
                   
                   ylim = c(min(USCitiesMarJul$dir_request, na.rm = TRUE), 
                            max(USCitiesMarJul$dir_request, na.rm = TRUE))) +
   # horizontal
...
We're going to introduce another `filter`ing method in this section to create the `TopCities` dataset.
...
```{r TopCities-geom_line-sol, fig.height=8, fig.width=10, out.height='100%', out.width='100%', message=FALSE, warning=FALSE}
TopCities %>% 
  ggplot(aes(x = date, y = dir_request, 
             group = trans_type, 
             color = trans_type)) + 
...
- set `"Original data uses 100 as baseline for usage at 2020-01-13"` to `subtitle`  
